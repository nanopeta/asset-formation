...
### 【致命的】load() 内で uid() を使ってはいけない（データ全消去バグ）
- `const uid=()=>...` は `let D=load()` より後に定義されているため、`load()` 内で `uid()` を呼ぶと `ReferenceError` が発生する
- try/catch に捕まり `makeDefault()` が返り、**ユーザーデータが全消去**される
- **ルール**: `load()` 内では `'sp'+Date.now()+index` などインラインIDを使う。`uid()` は絶対に使わない
- 発生日: 2026-05-07（spotList マイグレーションで発生）
...
### キャッシュバスターは app.js と style.css を必ず同時に上げる
- `index.html` 末尾の `?v=N` を片方だけ上げると古いファイルがキャッシュに残る
- **ルール**: どちらか一方でも変更したら `app.js?v=N` と `style.css?v=N` の両方を同じバージョン番号に更新する
- 発生日: 2026-05-07（style.css だけ v=7 に上げて app.js を忘れ、モバイルで変更が反映されなかった）
...
### 構造変更時は CLAUDE.md・README.md を必ず確認する
- セクション ID を追加・変更したら `CLAUDE.md` のタブ・セクション構成を更新する
- 主要機能を追加・削除したら `README.md` の機能一覧も確認する
- **ルール**: コミット前に `CLAUDE.md` / `README.md` の該当箇所に古い記述が残っていないかチェックする

---

## モバイル UX

### iOS では a.download + a.click() が動作しない
- iOS Safari はプログラム的な `<a download>` クリックをブロックする
- **正解パターン**: `navigator.canShare({files:[file]})` で Web Share API が使えるか確認し、使えれば `navigator.share()` でファイルを渡す。使えない場合のみ `a.click()` にフォールバック
- `_triggerExport(blob, filename, btnId)` ヘルパーがこのパターンを実装済み

### ユーザーへのフィードバックは alert() ではなく _flashBtn() を使う
- `alert()` はモバイルでブロッキングダイアログになり UX が悪い
- **正解パターン**: `_flashBtn(btnId)` でボタンを一時的に緑「✓ 完了」に変える（2秒後に元に戻る）
- 保存・エクスポートなど全ての操作完了フィードバックに統一する
