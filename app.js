...
function _applyRakutenRows(rows){if(!rows.length){toast('保有商品が見つかりませんでした','error');return;}const matched=[],toAdd=[];rows.forEach(row=>{const h=D.holdings.find(h=>{if(h.account!==row.account)return false;return h.name===row.name||row.name.includes(h.name)||h.name.includes(row.name);});if(h)matched.push({h,row});else toAdd.push(row);});if(!matched.length&&!toAdd.length){toast('処理対象の銘柄がありません','error');return;}let msg='【楽天証券CSV インポート確認】\n\n';if(matched.length){msg+='✅ 更新: '+matched.length+'件\n';matched.forEach(({h,row})=>{const old=D.current.holdingValues[h.id]?.value||0;msg+=`  ${h.name}: ${fmt(old)} → ${fmt(row.value)}\n`;});}if(toAdd.length){msg+='\n➕ 新規追加: '+toAdd.length+'件\n';toAdd.forEach(row=>msg+=`  ${row.name}（${row.accountStr}）\n`);msg+='  ※ 月次積立額・利回りは後で設定できます\n';}msg+='\n更新・追加してよろしいですか？';if(!confirm(msg))return;matched.forEach(({h,row})=>{D.current.holdingValues[h.id]={value:row.value,principal:row.principal};});toAdd.forEach(row=>{const nh={id:uid(),name:row.name,account:row.account,assetType:row.assetType,monthlyAmount:0,spotList:[],dividendYield:0,dividendMonths:[],order:D.holdings.length};D.holdings.push(nh);D.current.holdingValues[nh.id]={value:row.value,principal:row.principal};});persist();if(toAdd.length)renderSettings();renderDashboard();renderRecordTab();const parts=[];if(matched.length)parts.push(matched.length+'件更新');if(toAdd.length)parts.push(toAdd.length+'件追加');toast(parts.join('・')+'しました','success');}
