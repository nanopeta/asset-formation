...
function renderAnalysisData(){
    ...
    el('an-by-type').innerHTML=typeEntries.map(([k,d])=>breakdownRow(k==='cash'?'<span class="badge b-blue">現金</span>':atBadge(k),d,totalTypeVal,totalGain)).join('')+typeFooter;
    ...
    {const atypes=getAssetTypes();const typeColors=typeEntries.map(([k])=>k==='cash'?'#8ab4c8':(atypes[k]?.color||ASSET_TYPE_COLORS[k]||'#9ca3af'));const typeLabels=typeEntries.map(([k])=>k==='cash'?'現金':(getAssetTypes()[k]?.label||k));const typCtx=el('by-type-chart').getContext('2d');byTypeChart=_chartRender(byTypeChart,typCtx,{type:'doughnut',data:{labels:typeLabels,datasets:[{data:typeEntries.map(([,d])=>d.val),backgroundColor:typeColors,borderWidth:2,borderColor:'#fff'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{font:{size:11},boxWidth:11,padding:8}},tooltip:{callbacks:{label:c=>` ${fmt(c.raw)}  (${((c.raw/totalTypeVal)*100).toFixed(1)}%)`}}}}});}
