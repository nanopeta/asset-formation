...
function makeDefault(){
    return {
        ...
        holdings:[
            {id:'h-schd',  name:'楽天SCHD',                        account:'nisa-growth',    assetType:'fund',monthlyAmount:100000,spotAnnual:1200000,dividendYield:3.5,dividendMonths:[3,6,9,12],order:0},
            {id:'h-sp500', name:'eMAXIS Slim 米国株式(S&P500)',     account:'nisa-tsumitate', assetType:'fund',monthlyAmount: 50000,spotAnnual:      0,dividendYield:0.2,dividendMonths:[],order:1},
            {id:'h-orukan',name:'eMAXIS Slim 全世界株式(オルカン)',  account:'nisa-tsumitate', assetType:'fund',monthlyAmount: 50000,spotAnnual:      0,dividendYield:0.2,dividendMonths:[],order:2},
        ],
        idecoHoldings:[],
        customAccounts:[],
        customAssetTypes:[],
...
function openCardPanel(r=true){if(r){el('s-card-id').value='';el('s-card-name').value='';el('s-card-note').value='';el('s-card-panel-title').textContent='カードを追加';_buildCardBankOptions('');}el('s-card-panel').classList.add('open');}
...
// 保有銘柄
function renderHoldingsTable(){el('s-holdings-table').innerHTML=D.holdings.length===0?'<tr><td colspan="7" class="empty">銘柄なし</td></tr>':D.holdings.map(h=>{const sps=h.spotList||[],spTot=spotTotal(h),spDone=sps.filter(s=>s.done).length;const spCell=spTot>0?`${fmt(spTot)}<span style="color:var(--muted);font-size:11px;margin-left:3px;">✅${spDone}/${sps.length}</span>`:'--';return`<tr draggable="true" data-id="${h.id}" data-group="regular" ondragstart="dragStart(event)" ondragover="dragOver(event)" ondragleave="dragLeave(event)" ondrop="drop(event)" ondragend="dragEnd(event)"><td class="drag-handle">⠿</td><td>${h.name}</td><td>${acBadge(h.account)}</span></td><td>${atBadge(h.assetType)}</td><td style="text-align:right">${h.monthlyAmount>0?fmt(h.monthlyAmount)+'/月':'--'}</td><td style="text-align:right">${spCell}</td><td style="text-align:right">${h.dividendYield||0}%</td><td style="text-align:right"><div class="flex-gap" style="justify-content:flex-end"><button class="btn btn-s btn-sm" onclick="editHolding('${h.id}')">編集</button><button class="btn btn-d btn-sm" onclick="deleteHolding('${h.id}')">削除</button></div></td></tr>`;}).join('');}
...
function openHoldingPanel(r=true){if(r){el('s-holding-id').value='';el('s-h-name').value='';el('s-h-monthly').value='';el('s-h-yield').value='';renderSpotListPanel([]);el('s-holding-panel-title').textContent='銘柄を追加';buildAccountOptions('s-h-account','nisa-growth');buildAssetTypeOptions('s-h-type','fund');const cur=el('s-h-currency');if(cur)cur.value='jpy';setDivMonthsOnPanel([]);}el('s-holding-panel').classList.add('open');}
...
function _applyRakutenRows(rows){if(!rows.length){toast('保有商品が見つかりませんでした','error');return;}const matched=[],toAdd=[];rows.forEach(row=>{const h=D.holdings.find(h=>{if(h.account!==row.account)return false;return h.name===row.name||row.name.includes(h.name)||h.name.includes(row.name);});if(h)matched.push({h,row});else toAdd.push(row);});const matchedIds=new Set(matched.map(({h})=>h.id));const toDelete=D.holdings.filter(h=>!matchedIds.has(h.id));if(!matched.length&&!toAdd.length&&!toDelete.length){toast('処理対象の銘柄がありません','error');return;}const accs=getAccounts();let msg='【楽天証券CSV インポート確認】\n\n';if(matched.length){msg+='✅ 更新: '+matched.length+'件\n';matched.forEach(({h,row})=>{const old=D.current.holdingValues[h.id]?.value||0;msg+=`  ${h.name}: ${fmt(old)} → ${fmt(row.value)}\n`;});}if(toAdd.length){msg+='\n➕ 新規追加: '+toAdd.length+'件\n';toAdd.forEach(row=>msg+=`  ${row.name}（${row.accountStr}）\n`);msg+='  ※ 月次積立額・利回りは後で設定できます\n';}if(toDelete.length){msg+='\n🗑️ 削除（CSVに存在しない銘柄）: '+toDelete.length+'件\n';toDelete.forEach(h=>{const v=D.current.holdingValues[h.id]?.value||0;msg+=`  ${h.name}（${accs[h.account]?.label||h.account}）: ${fmt(v)}\n`;});}msg+='\n更新・追加・削除してよろしいですか？';if(!confirm(msg))return;matched.forEach(({h,row})=>{D.current.holdingValues[h.id]={value:row.value,principal:row.principal};});toAdd.forEach(row=>{const nh={id:uid(),name:row.name,account:row.account,assetType:row.assetType,monthlyAmount:0,spotList:[],dividendYield:0,dividendMonths:[],order:D.holdings.length};D.holdings.push(nh);D.current.holdingValues[nh.id]={value:row.value,principal:row.principal};});toDelete.forEach(h=>{D.holdings=D.holdings.filter(hh=>hh.id!==h.id);delete D.current.holdingValues[h.id];});persist();if(toAdd.length||toDelete.length)renderSettings();renderDashboard();renderRecordTab();const parts=[];if(matched.length)parts.push(matched.length+'件更新');if(toAdd.length)parts.push(toAdd.length+'件追加');if(toDelete.length)parts.push(toDelete.length+'件削除');toast(parts.join('・')+'しました','success');}
function importRakuten(e){const f=e.target.files[0];if(!f)return;e.target.value='';const r=new FileReader();r.onload=ev=>{try{const buf=ev.target.result;let rows=null,lastErr=null;for(const enc of['shift-jis','utf-8']){try{const text=new TextDecoder(enc).decode(buf);rows=_parseRakutenRows(text);break;}catch(e){lastErr=e;}}if(!rows)throw lastErr;_applyRakutenRows(rows);}catch(err){toast('CSVの読み込みに失敗しました: '+err.message,'error');}};r.readAsArrayBuffer(f);}
