...
function renderDashboard(){
    ...
    const planSeichouDsp=D.holdings.filter(h=>h.account==='nisa-growth'   ).reduce((a,h)=>a+spotDone(h),0);
    ...
    if(planEl){
        ...
        if(planSeichouM>0||planSeichouSp>0){
            ...
            let sp='';
            if(planSeichouSp>0){sp=` + スポット${fmt(planSeichouSp)}`;if(totalN>0)sp+=`<span class="spot-badge${doneN===totalN?' spot-done':''}">${doneN}/${totalN}件済</span>`;}
...
function saveCard(){const name=el('s-card-name').value.trim();if(!name){alert('カード名を入力してください');return;}const id=el('s-card-id').value;const bankId=el('s-card-bank')?.value||'';if(id){const cd=D.creditCards.find(cd=>cd.id===id);if(cd){cd.name=name;cd.note=el('s-card-note').value.trim();cd.bankId=bankId;}}else{const nc={id:uid(),name,note:el('s-card-note').value.trim(),bankId,order:D.creditCards.length};D.creditCards.push(nc);if(!D.current.cardValues[nc.id])D.current.cardValues[nc.id]=0;}persist();closeCardPanel();renderCardsTable();renderCardInputs();}
...
function addSpotRow(s){s=s||{id:uid(),amount:'',done:false};const row=document.createElement('div');row.className='spot-row';row.dataset.id=s.id;row.innerHTML=`<input type="number" class="hi spot-amount" value="${s.amount||''}" placeholder="金額（円）"><label class="spot-check"><input type="checkbox"${s.done?' checked':''}> ✅ 済</label><button type="button" class="btn btn-d btn-sm" onclick="this.closest('.spot-row').remove()">×</button>`;el('s-h-spot-list').appendChild(row);}
...
function init(){
    ...
    el('btn-auto-ideco').addEventListener('click',()=>{const est=calcIdecoEstimatedPri();if(!est){alert('基本設定でiDeCo開始月と月次拠出合計を設定してください');return;}el('rec-ideco-actual-pri').value=est;markUnsaved();});
