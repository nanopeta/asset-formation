...
function _parseRakutenRows(text){const lines=text.split(/\r?\n/);const di=lines.findIndex(l=>l.includes('保有商品詳細'));if(di===-1)throw new Error('保有商品詳細セクションが見つかりません');let hi=-1;for(let i=di;i<Math.min(di+10,lines.length);i++){const c=_parseCsvLine(lines[i]);if(c[0]==='種別'){hi=i;break;}}if(hi===-1)throw new Error('ヘッダー行が見つかりません');const unknownAT=new Set();const unknownAcc=new Set();const rows=[];for(let i=hi+1;i<lines.length;i++){const line=lines[i].trim();if(!line)break;const c=_parseCsvLine(line);const cat=c[0];if(!cat)continue;const value=_parseJpNum(c[14]);if(!value)continue;const gain=_parseJpNum(c[16]);const assetType=_RAKUTEN_TYPE_MAP[cat]||(unknownAT.add(cat),cat);const accountStr=c[3];const account=_RAKUTEN_ACC_MAP[accountStr]||(accountStr==='-'?'specific':(unknownAcc.add(accountStr),accountStr));rows.push({cat,name:c[2],accountStr,account,assetType,value,principal:value-gain});}let usdJpy=null;const fi=lines.findIndex(l=>l.includes('参考為替レート'));if(fi!==-1){for(let i=fi+1;i<Math.min(fi+15,lines.length);i++){const c=_parseCsvLine(lines[i]);if(c[0]==='米ドル'){usdJpy=parseFloat(c[1])||null;break;}}}let moneyBridgeBalance=null;const mbi=lines.findIndex(l=>l.includes('楽天銀行普通預金残高')||l.includes('マネーブリッジ残高'));if(mbi!==-1){const c=_parseCsvLine(lines[mbi]);const v=_parseJpNum(c[1]);if(v)moneyBridgeBalance=v;}let yokinBalance=null;const yki=lines.findIndex(l=>{const c=_parseCsvLine(l);return c[0]==='預り金';});if(yki!==-1){const c=_parseCsvLine(lines[yki]);const v=_parseJpNum(c[1]);if(v)yokinBalance=v;}return{rows,usdJpy,moneyBridgeBalance,yokinBalance,unknownAT:[...unknownAT],unknownAcc:[...unknownAcc]};}
function _applyRakutenRows({rows,usdJpy,moneyBridgeBalance,yokinBalance,unknownAT=[],unknownAcc=[]}){if(!rows.length&&!moneyBridgeBalance&&!yokinBalance){toast('保有商品が見つかりませんでした','error');return;}const newATMap={};unknownAT.forEach(label=>{const id=uid();newATMap[label]={id,obj:{id,label,badge:'b-gray',color:'#8888bb'}};});const newAccMap={};unknownAcc.forEach(label=>{const id=uid();newAccMap[label]={id,obj:{id,label,badge:'b-gray',color:'#aaaaaa',taxFree:false}};});rows.forEach(r=>{if(newATMap[r.assetType])r.assetType=newATMap[r.assetType].id;if(newAccMap[r.account])r.account=newAccMap[r.account].id;});const rakutenBroker=(D.brokers||[]).find(b=>b.name==='楽天証券');const rakutenBrokerId=rakutenBroker?.id||'';const matched=[],toAdd=[];rows.forEach(row=>{const h=D.holdings.find(h=>{if(h.account!==row.account)return false;return h.name===row.name||row.name.includes(h.name)||h.name.includes(row.name);});if(h)matched.push({h,row});else toAdd.push(row);});const matchedIds=new Set(matched.map(({h})=>h.id));const toDelete=D.holdings.filter(h=>!matchedIds.has(h.id)&&(!h.brokerId||h.brokerId===rakutenBrokerId));const rakutenBank=moneyBridgeBalance?(D.bankAccounts||[]).find(b=>b.name.includes('楽天')&&!b.name.includes('証券')&&!b.name.includes('預り')):null;const yokinAccount=yokinBalance?(D.bankAccounts||[]).find(b=>b.name.includes('預り')):null;if(!matched.length&&!toAdd.length&&!toDelete.length&&!usdJpy&&!moneyBridgeBalance&&!yokinBalance){toast('処理対象の銘柄がありません','error');return;}const accs=getAccounts();let msg='【楽天証券CSV インポート確認】\n\n';if(usdJpy){msg+=`💱 USD/JPY: ${D.settings.usdJpy||150} → ${usdJpy}\n\n`;}if(unknownAT.length){msg+=`🏷️ 新規銘柄種別作成: ${unknownAT.join('、')}\n`;}if(unknownAcc.length){msg+=`🗂️ 新規口座種別作成: ${unknownAcc.join('、')}\n`;}if(unknownAT.length||unknownAcc.length)msg+='\n';if(matched.length){msg+='✅ 更新: '+matched.length+'件\n';matched.forEach(({h,row})=>{const old=D.current.holdingValues[h.id]?.value||0;msg+=`  ${h.name}: ${fmt(old)} → ${fmt(row.value)}\n`;});}if(toAdd.length){msg+='\n➕ 新規追加: '+toAdd.length+'件\n';toAdd.forEach(row=>msg+=`  ${row.name}（${row.accountStr}）\n`);msg+='  ※ 月次積立額・利回りは後で設定できます\n';}if(toDelete.length){msg+='\n🗑️ 削除（CSVに存在しない銘柄）: '+toDelete.length+'件\n';toDelete.forEach(h=>{const v=D.current.holdingValues[h.id]?.value||0;msg+=`  ${h.name}（${accs[h.account]?.label||h.account}）: ${fmt(v)}\n`;});}if(moneyBridgeBalance){if(rakutenBank){const old=D.current.bankValues[rakutenBank.id]||0;msg+=`\n🏦 銀行残高: ${rakutenBank.name}: ${fmt(old)} → ${fmt(moneyBridgeBalance)}\n`;}else{msg+=`\n🏦 楽天銀行残高: ${fmt(moneyBridgeBalance)}（「楽天」を含む銀行口座が未登録のためスキップ）\n`;}}if(yokinBalance){if(yokinAccount){const old=D.current.bankValues[yokinAccount.id]||0;msg+=`\n💴 預り金: ${yokinAccount.name}: ${fmt(old)} → ${fmt(yokinBalance)}\n`;}else{msg+=`\n💴 預り金: ${fmt(yokinBalance)}（「預り金」を含む銀行口座が未登録のためスキップ）\n`;}}msg+='\n更新・追加・削除してよろしいですか？';const _safeHtml=msg.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');customConfirm(_safeHtml,()=>{if(usdJpy){D.settings.usdJpy=usdJpy;const uel=el('s-usd-jpy');if(uel)uel.value=usdJpy;}matched.forEach(({h,row})=>{D.current.holdingValues[h.id]={value:row.value,principal:row.principal};});toAdd.forEach(row=>{const nh={id:uid(),name:row.name,account:row.account,assetType:row.assetType,brokerId:rakutenBrokerId,monthlyAmount:0,spotList:[],dividendYield:0,dividendMonths:[],order:D.holdings.length};D.holdings.push(nh);D.current.holdingValues[nh.id]={value:row.value,principal:row.principal};});toDelete.forEach(h=>{D.holdings=D.holdings.filter(hh=>hh.id!==h.id);delete D.current.holdingValues[h.id];});if(moneyBridgeBalance&&rakutenBank){D.current.bankValues[rakutenBank.id]=moneyBridgeBalance;}if(yokinBalance&&yokinAccount){D.current.bankValues[yokinAccount.id]=yokinBalance;}Object.values(newATMap).forEach(({id,obj})=>{D.customAssetTypes.push(obj);if(!D.assetTypeOrder.includes(id))D.assetTypeOrder.push(id);});Object.values(newAccMap).forEach(({id,obj})=>{D.customAccounts.push(obj);if(!D.accountTypeOrder.includes(id))D.accountTypeOrder.push(id);});persist();if(toAdd.length||toDelete.length||unknownAT.length||unknownAcc.length)renderSettings();renderDashboard();renderRecordTab();const parts=[];if(usdJpy)parts.push('USD/JPY更新');if(matched.length)parts.push(matched.length+'件更新');if(toAdd.length)parts.push(toAdd.length+'件追加');if(toDelete.length)parts.push(toDelete.length+'件削除');if(moneyBridgeBalance&&rakutenBank)parts.push('銀行残高更新');if(yokinBalance&&yokinAccount)parts.push('預り金更新');if(unknownAT.length)parts.push(unknownAT.length+'種別作成');if(unknownAcc.length)parts.push(unknownAcc.length+'口座種別作成');toast(parts.join('・')+'しました','success');},{okLabel:'更新する',okClass:'btn-p',html:true});}
